...
} /* get_right_two_digits_of_current_year() */
...
} /* get_left_two_digits_of_current_year() */
...
int get_full_year_with_sed( char *sed_command, int optional_current_year )
{
	...
	p = popen( sys_str, "r" );
	fscanf( p, "%d", &full_year );
	pclose( p );
	return full_year;

} /* get_full_year_with_sed() */
