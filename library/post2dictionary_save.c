...
*/
	...
	{
		char decoded_data[ 65536 ];

		timlib_strcpy( decoded_data, input_buffer, 65536 );
				...
				char *dbms )
{
	...
	if ( target != -1 )
	{
		...
		{
			if ( dbms
			&&   strcmp( dbms, "oracle" ) == 0 )
			{
				strcpy(
				data,
				dictionary_encode_special(
					data ) );
			}
...
void post2dictionary_get_spool_filename(
						...
						char *session )
{
	if ( appaserver_mount_point && application_name && session )
	{
...
char *dictionary_encode_special( char *data )
{
	static char destination[ 65536 ];
	char *d = destination;
...
/*
	return (get_file_size( spool_filename ) != 0 );
*/
	return 1;
