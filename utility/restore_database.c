				...
				char delimiter )
{
	...
	static char sql_statement_process_string[ 4096 ];

	list_buffered_display(	column_name_list_string,
				column_name_list, 
				',' );

	sprintf( sql_statement_process_string,
				...
				char *configuration_filename )
{
	...
	*restore_table_list = local_restore_table_list;
	return 1;
} /* restore_database_load_configuration_file() */
...
int main( int argc, char **argv )
{
	...
	do {
		...
		system( restore_database->sql_statement_process_string );
