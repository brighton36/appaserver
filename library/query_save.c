...
{
	QUERY_OUTPUT *query_output;
	LIST *exclude_attribute_name_list = (LIST *)0;
/*
				...
				boolean include_root_folder )
{
	...
	{
		if ( ( query_drop_down =
			query_each_index_get_primary_data_drop_down(
				exclude_attribute_name_list,
	...
	do {
		...
		/* Drop-downs don't have a relational_operator. */
		/* -------------------------------------------- */
		if ( query_exists_relational_operator(
				related_folder->
					foreign_attribute_name_list,
				dictionary ) )
		{
			...
			DICTIONARY *dictionary )
{
	...
	if ( !list_rewind( attribute_name_list ) ) return 0;
...
} /* query_exists_relational_operator() */
				...
				char *login_name )
{
	...
	*ptr = '\0';
	...
	do {
		...
		if ( login_name && strcmp( attribute_name, "login_name" ) == 0 )
		{
			timlib_strcpy(
				escaped_data,
				login_name,
			...
			char *folder_name )
{
	...
	do {
		...
		&&  (strcmp( query_attribute->datatype, "time" ) == 0
		||   strcmp( query_attribute->
		...
		||     !folder_name )
		&&   query_attribute->relational_operator == between
		&&  (strcmp( query_attribute->datatype, "date" ) == 0
		||   strcmp( query_attribute->
				...
				char *folder_name )
{
	...
	*ptr = '\0';
	...
	do {
		...
		if ( ( strcmp( query_attribute->datatype, "float" ) == 0
		||     strcmp( query_attribute->datatype, "integer" ) == 0 )
		&&   query_attribute->relational_operator == between )
		{
			ptr += sprintf(
				ptr,
		...
		{
			if ( strcmp(	query_attribute->from_data,
					NOT_NULL_OPERATOR ) == 0 )
			{
				ptr +=
					sprintf(
					...
					char *data )
{
	...
	*where = '\0';
	...
	if ( folder_name  )
	{
		if ( strcmp( data, NULL_OPERATOR ) == 0 )
		{
			sprintf(where,
				QUERY_NULL_EXPRESSION,
			 	get_full_attribute_name(
		...
		if ( strcmp( data, NOT_NULL_OPERATOR ) == 0 )
		{
			sprintf(where,
				QUERY_NOT_NULL_EXPRESSION,
			 	get_full_attribute_name(
				...
				DICTIONARY *dictionary )
{
	...
	{
		query_drop_down =
			query_get_primary_data_row_drop_down(
				exclude_attribute_name_list,
				query_drop_down,
				root_folder_name,
				foreign_attribute_name_list,
...
{
char msg[ 65536 ];
sprintf( msg, "%s/%s()/%d: with index = %d, got dictionary_prepend_folder_name = %s\n",
__FILE__,
__FUNCTION__,
__LINE__,
index,
