			...
			char *serial_number )
{
	...
	select = purchase_tax_recovery_get_select();
	...
	sprintf( sys_string,
		 "get_folder_data	application=%s			"
		 "			select=%s			"
		 "			folder=tax_fixed_asset_recovery	"
		 "			where=\"%s\"			"
		 "			order=recovery_year		",
		 application_name,
		 select,
	...
	while( get_line( input_buffer, input_pipe ) )
	{
		...
		list_append_pointer( tax_recovery_list, tax_recovery );
	}
...
} /* purchase_fetch_tax_recovery_list() */

char *purchase_tax_recovery_get_select( void )
{
	char *select =
"full_name,street_address,purchase_date_time,asset_name,serial_number,recovery_year,recovery_amount,recovery_percent";

	return select;
}

TAX_RECOVERY *purchase_tax_recovery_new( void )
{
	TAX_RECOVERY *p =
		(TAX_RECOVERY *)
			calloc( 1, sizeof( TAX_RECOVERY ) );
...
} /* purchase_tax_recovery_new() */

TAX_RECOVERY *purchase_tax_recovery_parse( char *input_buffer )
{
	TAX_RECOVERY *tax_recovery;
	char piece_buffer[ 256 ];
	...
	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 0 );
	tax_recovery->full_name = strdup( piece_buffer );

	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 1 );
	tax_recovery->street_address = strdup( piece_buffer );

	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 2 );
	tax_recovery->purchase_date_time = strdup( piece_buffer );

	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 3 );
	tax_recovery->asset_name = strdup( piece_buffer );

	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 4 );
	tax_recovery->serial_number = strdup( piece_buffer );

	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 5 );
	tax_recovery->recovery_year = atoi( piece_buffer );

	piece( piece_buffer, FOLDER_DATA_DELIMITER, input_buffer, 6 );
	if ( *piece_buffer )
		tax_recovery->recovery_amount = atof( piece_buffer );
