				...
				int replicate_number )
{
	...
	sweep_list->non_primary_attribute_name_list =
		sweep_get_non_primary_attribute_name_list(
			1 /* with_total_minutes */ );

	sweep_list->sweep_list =
		sweep_get_sampling_point_sweep_list(
				application_name,
				anchor_date,
				anchor_time,
				location,
...
LIST *sweep_get_non_primary_attribute_name_list( boolean with_total_minutes )
{
	LIST *non_primary_attribute_name_list = list_new();

	list_append_pointer(	non_primary_attribute_name_list,
				"sweep_number" );
	list_append_pointer(	non_primary_attribute_name_list,
				"pick_date" );
	list_append_pointer(	non_primary_attribute_name_list,
				...
				LIST *non_primary_attribute_name_list )
{
	...
	search_replace_string(
		select_string,
		SWEEP_TOTAL_MINUTES_KEY,
		sweep_get_minutes_sum_expression() );
	...
	while( get_line( input_buffer, input_pipe ) )
	{
		piece(	piece_buffer,
			...
			input_buffer,
			sweep_non_primary_attribute_name_list_get_piece_offset(
				non_primary_attribute_name_list,
				"sweep_number" ) );
			...
			char *attribute_name )
{
	...
	if ( !list_rewind( non_primary_attribute_name_list ) )
	{
		fprintf( stderr,
		"ERROR in %s/%s()/%d: empty non_primary_attribute_name_list.\n",
			 __FILE__,
			 __FUNCTION__,
			 __LINE__ );
		exit( 0 );
...
char *sweep_get_minutes_sum_expression( void )
{
