		...
		LIST *purchase_order_list )
{
	...
	char *purchase_order_transaction_date_time = {0};

	if ( !list_rewind( purchase_order_list ) ) return;
	...
	do {
		...
		if ( !purchase_order->transaction ) continue;
		...
		if ( !purchase_order_transaction_date_time )
		{
		...
		/* If deleted the inventory purchase. */
		/* ---------------------------------- */
		if ( !purchase_order->sum_inventory_extension
		&&   debit_amount )
		{
			purchase_order_journal_ledger_refresh(
				application_name,
		...
		/* ---------------------------------- */
		/* If changed the inventory purchase. */
		/* ---------------------------------- */
		if ( !dollar_virtually_same(
			debit_amount,
			purchase_order->sum_inventory_extension ) )
		...
		/* If deleted the sales tax. */
		/* ------------------------- */
		if ( !purchase_order->sales_tax
		&&   debit_amount )
		{
			purchase_order_journal_ledger_refresh(
				application_name,
		...
		/* ------------------------- */
		/* If changed the sales tax. */
		/* ------------------------- */
		if ( !dollar_virtually_same(
			debit_amount,
			purchase_order->sales_tax ) )
		{
			/* Update sales_tax_expense */
			/* ------------------------ */
			ledger_journal_ledger_update(
				update_pipe,
				purchase_order->
		...
		/* If deleted the freight in. */
		/* -------------------------- */
		if ( !purchase_order->freight_in
		&&   debit_amount )
		{
			purchase_order_journal_ledger_refresh(
				application_name,
		...
		/* -------------------------- */
		/* If changed the freight in. */
		/* -------------------------- */
		if ( !dollar_virtually_same(
			debit_amount,
			purchase_order->freight_in ) )
		{
			/* Update freight_in_expense */
			/* ------------------------- */
			ledger_journal_ledger_update(
				update_pipe,
				purchase_order->
				...
				purchase_order->
					...
					transaction_date_time,
				freight_in_expense_account,
				purchase_order->freight_in /* debit_amount */,
				0.0 /* credit_amount */ );
		...
		/* ------------------------------- */
		/* If changed the account payable. */
		/* ------------------------------- */
		if ( !dollar_virtually_same(
			credit_amount,
			purchase_order->purchase_amount ) )
